...
TYPES:BEGIN OF WA_EMP,                  "ESTRUTURA GLOBAL"
        ...
        LV_SUBDECFER TYPE P DECIMALS 2, "DÉCIMO TERCEIRO + FÉRIAS"
...
PARAMETERS:
  P_NOME TYPE C LENGTH 20,  "NOME DO FUNCIONÁRIO"
  P_NIF  TYPE C LENGTH 9.   "NIF DO FUNCIONÁRIO"

LS_SAL-LV_NOME = P_NOME. "A VARIÁVEL DE MINHA ESTRUTURA RECEBE O VALOR DO PARÂMETRO"
LS_SAL-LV_NIF  = P_NIF.  "A VARIÁVEL DE MINHA ESTRUTURA RECEBE O VALOR DO PARÂMETRO"

SELECTION-SCREEN BEGIN OF BLOCK RADIO4 WITH FRAME TITLE TEXT-008. "SUBSÍDIO FÉRIAS E DÉCIMO TERCEIRO ORDENADO."
  PARAMETERS:
    PRB_NSDF TYPE BOOLEAN RADIOBUTTON GROUP GRP5 DEFAULT 'X', "SEM SUBSÍDIO E DÉCIMO PAGO NO SALÁRIO"
    PRB_YSDF TYPE BOOLEAN RADIOBUTTON GROUP GRP5.             "COM SUBSÍDIO E DÉCIMO PAGO NO SALÁRIO"

  "A ESCOLHA ENTRE ESTAS DUAS DECIDIRÁ SE O FUNCIONÁRIO RECEBE A PARCELA DO SUBSÍDIO MENSALMENTE"

SELECTION-SCREEN END OF BLOCK RADIO4.

SELECTION-SCREEN BEGIN OF BLOCK RADIO3 WITH FRAME TITLE TEXT-006. "TURNO DO FUNCIONÁRIO"
  PARAMETERS:
    PRB_DIAA TYPE BOOLEAN RADIOBUTTON GROUP GRP4, "TURNO DO DIA"
    PRB_NOIT TYPE BOOLEAN RADIOBUTTON GROUP GRP4. "TURNO DA NOITE"

  "A ESCOLHA ENTRE ESTAS DUAS DECIDIRÁ SE O FUNCIONÁRIO RECEBE ADICIONAL NOTURNO"

SELECTION-SCREEN END OF BLOCK RADIO3.

SELECTION-SCREEN BEGIN OF BLOCK RADIO1 WITH FRAME TITLE TEXT-001. "OPÇÕES DE CONTRATO"
  PARAMETERS:
  ...
  "A ESCOLHA ENTRE ESTAS DUAS VAI DIFERIR EM COMO O PROGRAMA VAI SE COMPORTAR EM RELAÇÃO ÀS COBRANÇAS À SEGURANÇA SOCIAL E IRS"
...
LS_SAL-LV_SALBASE = P_SALB. "A VARIÁVEL DE MINHA ESTRUTURA RECEBE O VALOR DO PARÂMETRO"

SELECTION-SCREEN BEGIN OF BLOCK RADIO2 WITH FRAME TITLE TEXT-003. "DUAS OPÇÕES DE TURNO PARA HORAS EXTRAS"
  PARAMETERS:
...
SELECTION-SCREEN BEGIN OF BLOCK CHECKBOXES WITH FRAME TITLE TEXT-002. "QUATRO OPÇÕES DE BENEFÍCIOS OFERECIDOS AO EMPREGADO PELA EMPRESA"
  PARAMETERS:
  ...
  "PARA CADA CHECKBOX PREENCHIDA UM VALOR REFERENTE A ESTES PRODUTOS É DEBITADO DO SALÁRIO LÍQUIDO OU ACRESCIDO"

SELECTION-SCREEN END OF BLOCK CHECKBOXES.

SELECTION-SCREEN BEGIN OF BLOCK CHECKBOXES4 WITH FRAME TITLE TEXT-004. "TRÊS OPÇÕES DE METAS BATIDAS"
  PARAMETERS:
...
IF PRB_YSDF = 'X'.                                                   "SE OPTAR POR RECEBER O SUBSÍDIO MENSALMENTE...
  LV_YSDF = LV_YSDF + ( LS_SAL-LV_SALBRUTO / 12 ) + ( P_SALB / 12 ). "A VARIÁVEL LOCAL RECEBE O SUBSÍDIO FÉRIAS + SUBSÍDIO DÉCIMO TERCEIRO"
ELSE.                                                                "DO CONTRÁRIO...
  LV_YSDF = 0.                                                       "A VARIÁVEL RECEBE ZERO.
ENDIF.

LS_SAL-LV_SUBDECFER = LV_YSDF. "A VARIÁVEL DE MINHA ESTRUTURA RECEBE O VALOR DO PARÂMETRO"
...
IF PRB_NOIT = 'X'.                                         "SE O TURNO DO EMPREGADO FOR NOTURNO"
  LS_SAL-LV_TURNO = 'NOTURNO'.                             "A VARIÁVEL DA MINHA ESTRUTURA RECEBE O NOME NOTURNO"
  LS_SAL-LV_BONUS = LS_SAL-LV_BONUS + ( '0.25' * P_SALB ). "E A VARIÁVEL BÔNUS DE MINHA ESTRUTURA RECEBE MAIS 25% DO SALÁRIO BASE"
ELSE.                                                      "DO CONTRÁRIO...
  LS_SAL-LV_TURNO = 'DIURNO'.                              "A VARIÁVEL DA MINHA ESTRUTURA RECEBE O NOME DIURNO"
ENDIF.
...
ELSE.                                                         "SE O FUNCIONÁRIO ATINGE O TERCEIRO NÍVEL DE META"
...
IF PRB_CONT = 'X'.                      "SE É UM FUNCIONÁRIO DE CONTRATO...
  LS_SAL-LV_TIPOCONTR = 'CONTRATO'.     "ESCREVE CONTRATO NA VARIÁVEL DA ESTRUTURA.
ELSE.                                   "DO CONTRÁRIO...
  LS_SAL-LV_TIPOCONTR = 'RECIBO VERDE'. "ESCREVE RECIBO VERDE NA VARIÁVEL DA ESTRUTURA.
  LS_SAL-LV_SUBDECFER = 0.              "E ZERA A VARIÁVEL QUE ARMAZENA A PARCELA DO SUBSÍDIO FÉRIAS E DÉCIMO TERCEIRO.
ENDIF.
...
LV_VALHRS = P_SALB / ( 8 * 22 ). "ESTA É UMA LÓGICA BÁSICA PARA SE CALCULAR QUANTO VALE A HORA EXTRA BASEADA NO SALÁRIO BASE DO EMPREGADO"
...
DATA: LV_SEGSOC TYPE P DECIMALS 2. "CRIEI UMA VARIÁVEL LOCAL PARA ARMAZENAR O VALOR DEBITADO PELA SEGURANÇA SOCIAL"
DATA: LV_IRS    TYPE P DECIMALS 2. "CRIEI UMA VARIÁVEL LOCAL PARA ARMAZENAR O VALOR DEBITADO DE IRS"
...
LS_SAL-LV_SEGSOC = LV_SEGSOC. "A VARIÁVEL DA ESTRUTURA RECEBE A VARIÁVEL LOCAL DO VALOR DA SEGURANÇA SOCIAL"
LS_SAL-LV_IRS    = LV_IRS.    "A VARIÁVEL DA ESTRUTURA RECEBE A VARIÁVEL LOCAL DO VALOR DO IRS"
...
"CÁLCULO DE METAS REFERENTES AO NÍVEL DE META ALCANÇADO."
...
LS_SAL-LV_SALIQ = LS_SAL-LV_SALBRUTO - LS_SAL-LV_DEDUCT + LS_SAL-LV_BONUS + LS_SAL-LV_SUBDECFER. "SALÁRIO LÍQUIDO É O SALÁRIO BRUTO MENOS AS DEDUÇÕES SALARIAIS(CARGAS TRIBUTÁRIAS E SERVIÇOS).
